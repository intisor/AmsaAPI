...
namespace AmsaAPI.Endpoints
{
    public static class OrganizationEndpoints
    {
        ...
        {
            // Unit endpoints
            var unitGroup = app.MapGroup("/api/units").WithTags("Units");
            ...
            // State endpoints
            var stateGroup = app.MapGroup("/api/states").WithTags("States");
            stateGroup.MapGet("/", GetAllStates);
            stateGroup.MapGet("/{id:int}", GetStateById);
            stateGroup.MapPost("/", CreateState);

            // National endpoints
            var nationalGroup = app.MapGroup("/api/nationals").WithTags("Nationals");
            nationalGroup.MapGet("/", GetAllNationals);
            nationalGroup.MapGet("/{id:int}", GetNationalById);

            // Hierarchy endpoint
            app.MapGet("/api/hierarchy", GetHierarchy).WithTags("Organization");
        }
        ...
        private static async Task<IResult> CreateUnit(Unit unit, AmsaDbContext db)
        {
            try
            {
                ...
                return Results.Created($"/api/units/{unit.UnitId}", 
        ...
        private static async Task<IResult> CreateState(State state, AmsaDbContext db)
        {
            try
            {
                ...
                return Results.Created($"/api/states/{state.StateId}", 
